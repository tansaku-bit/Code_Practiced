...
+ オブジェクト  
  ...
  //オブジェクトnew_objectは空のオブジェクトなので何も表示されない
  ~~~
  オブジェクトの宣言方法は変数や定数と同じ要領だが、中括弧（{}）でプロパティをまとめる形で宣言する。  
  1つのプロパティはキー、コロン（:）、値で宣言し、プロパティとプロパティはカンマ（,）で区切る。 
  プロパティの値は単純な値や文字列だけでなく式としての記述も可能。  
  オブジェクトのプロパティへアクセスするためには、ピリオド演算子形式を用いる。大括弧の形式でアクセスすることも可能だが、ピリオド演算子形式の方が主流になる。

  **Objectオブジェクト**
  + hasOwnPropertyメゾット
    オブジェクトが指定したプロパティを持っているか調べるために利用されるメゾットで、`オブジェクト名.hasOwnProperty(キー)`を用いる。この時、存在するならばtrue（真）が返され、存在しない場合はfalse（偽）が返される。
  + for…in文
    オブジェクトに含まれるプロパティのプロパティ名を順に取り出しながら繰り返し処理を行う書式。`for(変数名 in オブジェクト){}`で利用可能。
  + keysメゾット
    オブジェクトに含まれるプロパティのプロパティ名の一覧を配列として取得するメゾットで、`object.keys(オブジェクト名)`を用いる。
  + valuesメゾット
    オブジェクトに含まれるプロパティの値の一覧を配列として取得するメゾットで、`object.values(オブジェクト名)`を用いる。
  + entriesメゾット
    オブジェクトに含まれるキーと値のペアの一覧を配列として取得するメゾットで、`object.entries(オブジェクト名)`を用いる。
  + forEntriesメゾット
    プロパティと値のデータから御ぶいぇくとを作成するメゾットで、`object.fromEntries(オブジェクト名)`を用いてオブジェクトが生成される。  
    値が1つしかない場合、プロパティの値はundefinedとなる。
  + freezeメゾット
    オブジェクトに対する変更を禁止するメゾットで、`object.freeze(オブジェクト名)`を用いる。メゾットを実行するとプロパティの追加、削除、変更のいずれもできなくなる。 
    このfreezeメゾットは配列に対しても有効で、`object.freeze(配列名)`を用いれば配列でも同様の処理が得られる。なお、変更しようとしてもその処理が有効にならないだけでエラーが出るわけではない。
  + assignメゾット
    複数のオブジェクトを1つのオブジェクトにまとめるメゾットで、`object.assign(コピー先オブジェクト, コピー元オブジェクト1, …, コピー元プロジェクトn)`を用いる。コピー元のオブジェクトにプロパティが追加されていく。 
    このメゾットの応用でオブジェクトの複製が可能で、コピー元のオブジェクトを空のオブジェクト（{}）にする。`let clone_obj = object.assign({}, origin_obj)`を用いる。
...
## 算術演算子
  基本的な文法を学び実践する上で良く使用する者のみ記載。演算子全てはリファレンスを参照すること。
| 演算子 | 記法 | 使用例 | 演算の優先度 |
| ----- | ---- | ------ | ---------- |
| 加法演算子 | + | x + y |  |
| 減法演算子 | - | x - y |  |
